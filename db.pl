...
%Ho provato qualche query semplice con https://swish.swi-prolog.org/
%?-movie_title(1099,T). -> T = 'The Blind Side'
%?-movie_genre(1099,G). -> G = sport
%?-movie_year(1099,Y). -> Y = 2009
%?-movie(T). -> Ritorna il titolo di tutti i film
%?-movie_per_genre( T,'action'). -> Ritorna il titolo di tutti i film di genere 'action'
...
%Ho provato qualche query semplice con https://swish.swi-prolog.org/
%?-tvshow_title(2029,T). -> T = 'Mindhunter'
%?-tvshow_genre(2030,G). -> G = fantasy
%?-tvshow_network(2030,N). -> N = 'HBO'
%?-tvshow(T). -> Ritorna il titolo di tutti i tv shows
%?-tvshow_per_genre(T,drama). -> Ritorna il titolo di tutti i tv show di genere 'drama'
%?-tvshow_per_network(T,'HBO') -> Ritorna il titolo di tutti i tv show che hanno per network 'HBO'
%?-display_tvshow(2029). -> [2029, Mindhunter, drama, Netflix]